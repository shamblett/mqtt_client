...
class MqttWebInitializer {
  ...
  void initClient({
    ...
    _client = MqttServerClient(params.url, '');
...
void main() async {
  final server = MqttWebInitializer();
  server.initClient(params: MqttParams());
  await server.connect();
  server.subscribe('my/dashboard/topic/chatList/#', qos: MqttQos.exactlyOnce);
  await Future.delayed(Duration(seconds: 10));
  server.unsubscribe('my/dashboard/topic/chatList/#');
  server.disconnect();
}
